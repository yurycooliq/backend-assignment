...
export class SessionService {
  ...
  async createSession(
    ...
    const session = await this.sessionsRepository.create(
      { brandId, userId, tokenHash, expiresAt },
      client,
    );
  ...
  async authenticate(brandId: string, accessToken: string): Promise<Session> {
    const tokenHash = this.hashToken(accessToken);
    const session = await this.sessionsRepository.findByTokenHashForBrand(
      brandId,
      tokenHash,
    );
    ...
    const tokenBrand =
      await this.sessionsRepository.findBrandByTokenHash(tokenHash);

    if (tokenBrand && tokenBrand.brandId !== brandId) {
      throw ApplicationError.forbidden(
        "TENANT_MISMATCH",
        "Session belongs to a different brand",
      );
    }
