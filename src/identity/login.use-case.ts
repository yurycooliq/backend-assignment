...
export class LoginUseCase {
  ...
  async execute(brandId: string, dto: AuthDto): Promise<AuthResult> {
    ...
    if (!user) {
      throw ApplicationError.unauthorized('INVALID_CREDENTIALS', 'Invalid email or password');
    }
    ...
    const { accessToken, session } = await this.sessionService.createSession(brandId, user.id);
