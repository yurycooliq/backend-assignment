...
export class WebhookController {
  ...
  @Post('psp/:provider')
  @ApiResponse({ status: HttpStatus.ACCEPTED, description: 'First PSP callback accepted' })
  ingestPsp(
    ...
    return this.ingest(CallbackSource.PSP, provider, payload, request, response);
  }

  @Post('gsp/:provider')
  @ApiResponse({ status: HttpStatus.ACCEPTED, description: 'First GSP callback accepted' })
  ingestGsp(
    @Param('provider') provider: string,
    @Body() payload: unknown,
    @Req() request: RequestWithContext,
    ...
    return this.ingest(CallbackSource.GSP, provider, payload, request, response);
  }
